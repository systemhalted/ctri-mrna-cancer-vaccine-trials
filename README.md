...
- `update_reports.py`: turns a collector sweep into the committed automated report.
- `.github/workflows/update-reports.yml`: weekly sweep that opens a pull request when the results change.
- `data/automated/`: generated by that workflow. Machine output, not curated research.
- `reports/AUTOMATED_SWEEP.md`: generated summary of the most recent sweep.
- `tests/test_classifier.py`: classifier and parser tests.
- `tests/test_update_reports.py`: report-generation tests.
...
`.github/workflows/update-reports.yml` runs the sweep every Monday at 06:15 UTC,
and on demand from the Actions tab. Each run:

1. runs the unit tests, and stops there if they fail -- a sweep classified by a
   broken classifier is worse than no sweep;
2. sweeps the ClinicalTrials.gov v2 API for every term in `search_terms.txt`,
...
- **It is deterministic.** Nothing in the generated files depends on the wall
  ...
  accumulate empty refresh commits. The date of a no-change check is recorded in
  the workflow run summary instead.
- **It fails loudly rather than quietly emptying the report.** `--min-records`
...
it. The workflow therefore covers exactly what the CLI covers.

To run the same refresh by hand:

```bash
python ctri_mrna_trials.py --search-terms-file search_terms.txt --output-dir output
python update_reports.py --sweep-json output/raw_results.json
```

Add manually discovered CTRI records to the same report by passing them to the
collector first:
...
python update_reports.py \
  --sweep-json /tmp/ctri-fixture-output/raw_results.json \
  --data-dir /tmp/ctri-fixture-output/automated \
  --report /tmp/ctri-fixture-output/AUTOMATED_SWEEP.md \
  --source "synthetic fixture"
```

The fixture is synthetic and must never be committed as report data; point the
report generator somewhere outside the repository when running it this way.
...
| Research data and documentation (`data/`, `docs/`, `reports/`, `FINDINGS.md`, `VALIDATION.txt`, `search_terms.txt`) | **CC BY 4.0** | [`LICENSE-DATA`](LICENSE-DATA) |
